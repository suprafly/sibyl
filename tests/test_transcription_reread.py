...
class FakeLineLocalizer:
    ...
    def localize(self, image: Image.Image) -> tuple[dict[str, Any], float]:
        self.images.append(image)
        self.observer({"message": {"content": "line localization raw"}})
        return {
            "text_lines": [
                {"bbox_2d": [500, 500, 900, 700]},
...
def test_line_bbox_validation_and_ordering_are_strict() -> None:
    accepted, rejected = validate_line_regions(
        [
            {"bbox_2d": [500, 500, 900, 700]},
            {"bbox_2d": [100, 100, 400, 300]},
            {"bbox_2d": [1, 2, 3, 4, 5]},
    ...
    assert [item["bbox_2d"] for item in sorted(accepted, key=lambda item: item["bbox_2d"][1])] == [
        [100.0, 100.0, 400.0, 300.0],
        [500.0, 500.0, 900.0, 700.0],
    ]
...
def test_line_localizer_has_relative_frame_and_dedicated_controls(monkeypatch: Any) -> None:
    captured: dict[str, Any] = {}
    ...
    class Response:
        ...
        def read(self) -> bytes:
            return json.dumps(
                {"message": {"content": '{"text_lines": [{"bbox_2d": [1, 2, 3, 4]}]}'}},
            ).encode()
    ...
    monkeypatch.setattr(urllib.request, "urlopen", urlopen)
    result, _duration = OllamaTextLineLocalizer(model="test", base_url="http://test").localize(
        Image.new("RGB", (4, 4))
    )
    payload = json.loads(captured["request"].data)
    assert result == {"text_lines": [{"bbox_2d": [1, 2, 3, 4]}]}
    assert payload["options"]["num_predict"] == LINE_LOCALIZATION_NUM_PREDICT == 256
    assert payload["think"] is False
    assert payload["stream"] is False
    assert payload["keep_alive"] == 0
    assert payload["format"] == LINE_LOCALIZATION_SCHEMA
    assert "relative to this crop" in LINE_LOCALIZATION_PROMPT


def test_truncated_line_localization_preserves_raw_response(monkeypatch: Any) -> None:
    body = {
        "done_reason": "length",
        "message": {"thinking": '{"text_lines": [{"bbox_2d": [1, 2, 3, 4]}'},
    }
...
def test_region_first_reuses_one_prepared_image_and_exact_rgb_crop(tmp_path: Path) -> None:
    ...
    result = run_reread_experiment(
        ...
        line_localizer_factory=FakeLineLocalizer,
    ...
    assert len(FakeReader.instances) == 3
    assert len(FakeReader.instances[0].images) == 5
    assert len({id(image) for image in FakeReader.instances[0].images}) == 1
    assert len(result["regions"][0]["line_localization"]["regions"]) == 2
    assert result["regions"][0]["line_localization"]["regions"][0]["line_id"] == "region-01-line-01"
    assert result["regions"][0]["line_localization"]["regions"][0]["source_bbox"] == {
        "left": 11,
        "top": 11,
        "right": 18,
        "bottom": 16,
    }
    assert result["regions"][0]["line_localization"]["raw_response"] == {
        "message": {"content": "line localization raw"}
    }
    line_path = result["regions"][0]["line_localization"]["regions"][0]["path"]
    with Image.open(line_path) as line_crop:
        assert line_crop.mode == "RGB"
        assert line_crop.size == (7, 5)
        assert line_crop.getpixel((0, 0)) == (12, 34, 56)
    assert len(FakeReader.instances[1].images) == 5
    assert len({id(image) for image in FakeReader.instances[1].images}) == 1
