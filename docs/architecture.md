...
Sibyl is a Python CLI-first local tool. It separates source evidence, transformed
structure, interpretation, vocabulary evidence, and projection. The durable
semantic object is a transform result structure, not Markdown. Projection
...
- Source evidence owns original scans/photos and deterministic references to
  source regions.
- Transformed structure owns text, uncertain or uninterpreted regions, drawings,
  diagrams, ordering, relationships, and provenance.
- Interpretation may propose transformed meaning, but models are assistants and
...
- Projection converts transformed structure to targets such as Markdown/Obsidian
  or JSON without changing the transform model.
- CLI/application orchestration owns command parsing and future use-case
  composition; it does not become the owner of transform semantics.
...
2. Single-page transform: one genuinely difficult handwritten page, including
   drawings and uncertain handwriting, transformed into structure and emitted to
