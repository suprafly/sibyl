# Qwen3-VL benchmark transform extension

Update the existing `sibyl run IMAGE` implementation in place using the
...
check/audit and git diff --check. Do not redesign transform, add the real
